...
export default function Navbar() {
  ...
  const activeCss =
    "bg-[#a1edf7] h-16 w-45 flex items-center justify-center  text-black rounded-full px-10";
  return (
    <>
        ...
        <div className="w-full h-full flex items-center justify-between md:justify-evenly px-6 md:px-12 rounded-[60px] shadow-[0_0_60px_rgba(31,229,255,0.8)] bg-[#1FE5FF]">
          ...
          <div className="hidden md:flex items-center justify-center">
            <img
              src="/assets/logo.png"
              alt="Logo"
              className="h-20 md:h-24 lg:h-[130px] w-auto object-contain"
            />
          </div>
          ...
          >
            <span
              className={`w-6 h-0.5 bg-[#0D0D0D] transition-all ${isMenuOpen ? "rotate-45 translate-y-2" : ""}`}
            ></span>
            <span
              className={`w-6 h-0.5 bg-[#0D0D0D] transition-all ${isMenuOpen ? "opacity-0" : ""}`}
            ></span>
            <span
              className={`w-6 h-0.5 bg-[#0D0D0D] transition-all ${isMenuOpen ? "-rotate-45 -translate-y-2" : ""}`}
            ></span>
      ...
      {/* Mobile Sidebar Menu */}
      <div
        className={`fixed top-0 right-0 h-full w-64 bg-[#1FE5FF] shadow-2xl z-[60] transform transition-transform duration-300 ${isMenuOpen ? "translate-x-0" : "translate-x-full"} md:hidden`}
      >
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`hover:text-white transition-colors text-2xl font-normal py-2 border-b border-[#0D0D0D]/20 ${
                  isActive(link.href)
                    ? "bg-[#0D0D0D] text-white rounded-lg px-4"
                    : ""
                }`}
                style={{
